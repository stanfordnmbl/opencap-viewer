  ...
  <div class="step-5 d-flex">
    <div class="left d-flex flex-column pa-2">
      ...
      <div class="trials flex-grow-1">
        <div 
          v-for="(t, index) in filteredTrials"
          :key="`trial-${index}`"
          class="my-1 trial"
  ...
  v-show="show_controls"
    ...
    >
      ...
      <v-btn
        ...
        v-on="on"
  v-show="show_controls"
        class="mt-4">Share on <v-icon aria-hidden="false">
  mdi-facebook
  ...
  mdi-linkedin
      ...
      <v-card>
        ...
        <v-card-text>
          <v-checkbox
            id="session-public"
            v-model="session.public"
            name="session-public"
            label="Make session public"
            @change="setPublic($event)"
            />
          <v-container
            v-show="session.public"
            >
            <h3 class="mb-2">Share on</h3>
        <ShareNetwork
            network="facebook"
            class="mr-2" style="text-decoration: none;"
            :url="sessionUrl"
        ...
        <ShareNetwork
            network="linkedin"
            :url="sessionUrl" style="text-decoration: none;"
            title="OpenCap session">
            <v-btn><v-icon aria-hidden="false">mdi-linkedin</v-icon> &nbsp;LinkedIn</v-btn>
        </ShareNetwork>
  
          <v-text-field
            label="Alternatively, copy the session link and share on social media"
            v-model="sessionUrl"
            class="mt-5"
            readonly
            ></v-text-field>
